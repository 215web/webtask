...
dim DEFAULT_ASP_CHUNK_SIZE:DEFAULT_ASP_CHUNK_SIZE = Setting(6)
Class FreeASPUpload
	...
	Public Sub Save(path)
		Dim streamFile, fileItem,FileExtName,RandomFigure,NFileName,RndStr,n
		...
		For Each fileItem In UploadedFiles.Items
			...
			FileExtName=split(fileItem.FileName,".")(1)
			NFileName= Year(Now())&Right("0"&Month(Now()),2)&Right("0"&Day(Now()),2)&Right("0"&Hour(Now()),2)&Right("0"&Minute(Now()),2)&Right("0"&Second(Now()),2)&RandomFigure&"."&FileExtName
			
'Response.Write(Upload.Form("enter_a_number"))
'Response.end()
			streamFile.SaveToFile path & NFileName, 2
			streamFile.close
