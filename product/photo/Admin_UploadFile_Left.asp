...
if UploadDir="" then UploadDir = "uploadimages"
...
Response.Write "<title>Management navigation menu</title>" & vbCrLf
...
Response.Write "</head>" & vbCrLf
Response.Write "<BODY leftmargin='0' topmargin='0' marginheight='0' marginwidth='0' bgcolor='#f1f1f1' class='hback' style='font-size:12px;' >" & vbCrLf
...
Else
    FoundErr = True
    Response.Write "未能获取上传目录,请关闭此窗口,再重新打开即可！"
    Response.End
End If
...
Response.Write "          <td height='28'>上传目录：</td>" & vbCrLf
Response.Write "        </tr><tr>" & vbCrLf
Response.Write "          <td><img src='Images/Folder/folderopen.gif' width='18' height='18'>" & vbCrLf
Response.Write "          <a href='Admin_UploadFile_Main.asp?ChannelID=" & ChannelID & "&UploadDir=" & UploadDir & "' target=UploadFile_Main><span Path='" & UpFilesPath & "'  class='TempletItem'>" & UploadDir & "</span></a></td>" & vbCrLf
Response.Write "        </tr>" & vbCrLf
...
For Each FolderItem In SubFolderObj
    ...
    End If
        ...
        Response.Write "       <td  style='font-size:12px;'><a href='Admin_UploadFile_Main.asp?ChannelID=" & ChannelID & "&UploadDir=" & UploadDir & "&CurrentDir=" & FolderItem.name & "' target=UploadFile_Main><span Path='" & UpFilesPath & FolderItem.name & "' class='TempletItem'>" & FolderItem.name & "</span</a></td>"
        Response.Write "     </tr>"
...
Response.Write "</table>" & vbCrLf
Response.Write "<br><a href='newfolder.asp'><font color='#0000ff'>点击新建文件夹</font></a><br>如将全部图片放到一起，可能太多了，影响打开速度。您可以新建多个文件夹，然后将图片批量上传到不同的文件夹下。<br>" & vbCrLf
Response.Write "</td>" & vbCrLf
Response.Write "  </tr>" & vbCrLf
Response.Write "</table>" & vbCrLf
...
Function GetCurrentPath(Path)
    ...
    Else
        tempArr = Split(Path, "/")
        If SysRootDir <> "" Then
            GetCurrentPath = "&ParentDir=" & Replace(Replace(Path, "/" & tempArr(UBound(tempArr)), ""), "/" & tempArr(1) & "/"&UploadDir&"/", "") & "&CurrentDir=" & tempArr(UBound(tempArr))
        Else
