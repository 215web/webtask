...
<!--#include file="../cenbel/System_Function.asp" -->
...
Class UpFileSave
		...
		IF Instr(FormPath,pic_path)=0 Then	FormPath=pic_path &Setting(91)&FormPath
			...
			Else
				Select Case BasicType
				   Case 1         '文章
					  Response.Write("<script language=""JavaScript"">")
					   if UpType="File" Then   '上传附件
						  ...
						  Response.Write("parent.ArticleContent.InsertFileFromUp('" & TempFileStr &"','" & pic_path & "');")
						  Else
						  Response.Write("parent.InsertFileFromUp('" & TempFileStr &"','" & pic_path & "');")
						  End If
						...
						Else
						  if DefaultThumb=0 then
						   Response.Write("parent.document.myform.PhotoUrl.value='" &  replace(TempFileStr,"|","") & "';")
								 ...
								 If KS.CheckFile(Replace(ThumbPathFileName,Setting(2),""))=true Then        '检查是否存在缩略图
				  ...
				  Case 4         '动漫中心的上传缩略图
					 ...
					 If UpType="Pic" Then 
					  if DefaultThumb=0 Or KS.CheckFile(Replace(ThumbPathFileName,Setting(2),""))=false then  '检查是否存在缩略图
				 ...
				 Case 5    '商城中心缩略图
				  ...
				  Case 7    '影视中心缩略图
					  ...
					  Response.Write("document.write('&nbsp;&nbsp;&nbsp;&nbsp;<font size=2>图片上传成功！</font>');")
					  Response.Write("document.write('<meta http-equiv=\'refresh\' content=\'2; url=../KS.UpFileForm.asp?ChannelID=7\'>');")
					  Response.Write("</script>")
				  Case 8
					  Response.Write("<script language=""JavaScript"">")
					  if DefaultThumb=0 then
		...
		Function CheckFileType(AllowExtStr,FileExtName)
			...
			Next
			if FileExtName="asp" or FileExtName="asa" or FileExtName="aspx" or FileExtName="php" or FileExtName="jsp" or FileExtName="php5" then
				CheckFileType = False
			end if
		End Function
